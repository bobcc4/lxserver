...
- **洗版换源控制**：洗版页面新增“不换源”和“不换平台”选项。前者限制在当前用户选中的音源解析脚本内重试，后者限制使用原歌曲平台，选项按用户保存。
- **洗版结果显示大小**：处理结果显示原文件和替换文件的实际大小，例如“无损音质（42.18 MB）→ 母带音质（86.52 MB）”；跳过和降级结果也显示可获得的大小。
- **Subsonic 本地音乐目录**：音流等 Subsonic 客户端新增“本地音乐”目录，可浏览和搜索服务器 `music/<用户名>` 下载目录中的歌曲，并直接播放对应文件。

### Bug 修复

- **Subsonic 下载歌曲不可见**：修复歌曲已经下载到服务器 `music` 目录，但未加入同步歌单时，音流无法读取、搜索或播放的问题。
