...
// Pure TypeScript QRC decoder adapted for Node.js from
// any-listen/any-listen-extension-online-metadata (Apache-2.0).
...
export const decodeQrc = async (hexData: string): Promise<string> => {
  ...
  const encrypted = new Uint8Array(Buffer.from(hexData, 'hex'))
  ...
  try {
    return inflateSync(encrypted, { finishFlush: zlibConstants.Z_SYNC_FLUSH }).toString('utf8')
  } catch {
    return ''
  }
}
